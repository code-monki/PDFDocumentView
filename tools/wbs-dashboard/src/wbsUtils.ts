...
export function nextAttentionHint(counts: Record<TaskStatus, number>): string {
  if (counts.blocked > 0) {
    return `${counts.blocked} blocked task(s) — resolve or document before shipping momentum.`;
  }
  if (counts["in-progress"] > 0) {
    return `${counts["in-progress"]} in progress — close or re-scope before picking up deferred work.`;
  }
  if (counts.deferred > 0) {
    return "No blocked items; deferred backlog exists — schedule when prerequisites (backend, API freeze) land.";
  }
  return "All tracked items are done for this snapshot — verify docs/concept-pdf-document-view.md and docs/adrs/ for authoritative status.";
}
