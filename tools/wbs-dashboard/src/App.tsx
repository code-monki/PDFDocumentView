...
/** Default WBS status checkboxes — includes Done so completed tasks stay visible on first load. */
...
const STATUS_FILTER_STORAGE_KEY = "pdfdocumentview-wbs-dashboard.status-filter-v1";

function loadStatusFilterFromStorage(): Set<TaskStatus> {
  if (typeof window === "undefined") return new Set(DEFAULT_STATUSES);
  try {
    const raw = localStorage.getItem(STATUS_FILTER_STORAGE_KEY);
    // Missing key: full default (includes Done).
    if (raw == null) return new Set(DEFAULT_STATUSES);
    ...
    const arr: unknown[] | null = Array.isArray(parsed)
      ...
      : parsed &&
        ...
        : null;
    if (!arr) return new Set(DEFAULT_STATUSES);
    const next = new Set<TaskStatus>();
    for (const x of arr) {
      if (DEFAULT_STATUSES.includes(x as TaskStatus)) next.add(x as TaskStatus);
    }
    return next.size > 0 ? next : new Set(DEFAULT_STATUSES);
  } catch {
...
export default function App() {
  ...
  const [statusFilter, setStatusFilter] = useState<Set<TaskStatus>>(loadStatusFilterFromStorage);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        ...
        JSON.stringify({ statuses: [...statusFilter] }),
      );
    } catch {
      /* quota / private mode */
    }
      ...
      <div className={styles.shell}>
        <header className={styles.header}>
          ...
          <p className={styles.sub}>
            Offline-first WBS for the pdf-document-view widget library — data from
            checked-in JSON under <code>public/</code>.
          </p>
