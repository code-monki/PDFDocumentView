...
function(_pdfdocumentview_apply_pdfium_root root_dir)
    ...
    elseif(WIN32 AND EXISTS "${root_dir}/bin/pdfium.dll")
        ...
        if(EXISTS "${root_dir}/lib/pdfium.dll.lib")
            set(_lib "${root_dir}/lib/pdfium.dll.lib")
        elseif(EXISTS "${root_dir}/lib/pdfium.lib")
...
function(_pdfdocumentview_pdfium_host_cpu out_var)
    ...
    # Native builds: GitHub-hosted runners sometimes report inconsistent CMAKE_(HOST_)SYSTEM_PROCESSOR
    # values relative to the job's userland architecture. Prefer `uname -m` for Unix hosts, with a
    # Windows fallback when Git Bash isn't available to CMake's execute_process.
    if(CMAKE_SYSTEM_NAME STREQUAL "Linux" OR CMAKE_SYSTEM_NAME STREQUAL "Darwin")
        execute_process(
            COMMAND uname -m
            OUTPUT_VARIABLE _pdfdocumentview_uname_m
            OUTPUT_STRIP_TRAILING_WHITESPACE
            COMMAND_ERROR_IS_FATAL ANY)
        set("${out_var}" "${_pdfdocumentview_uname_m}" PARENT_SCOPE)
        return()
    endif()
    if(WIN32)
        execute_process(
            ...
            OUTPUT_STRIP_TRAILING_WHITESPACE
            ERROR_QUIET)
        if(_pdfdocumentview_uname_m STREQUAL "")
            set(_pdfdocumentview_uname_m "${CMAKE_HOST_SYSTEM_PROCESSOR}")
        endif()
        set("${out_var}" "${_pdfdocumentview_uname_m}" PARENT_SCOPE)
        return()
    endif()
    set("${out_var}" "${CMAKE_HOST_SYSTEM_PROCESSOR}" PARENT_SCOPE)
endfunction()
...
elseif(PDFDOCUMENTVIEW_FETCH_PDFIUM AND APPLE)
    _pdfdocumentview_pdfium_host_cpu(_cpu)
