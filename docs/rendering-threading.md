...
- **Rasterization (PDFium):** **Live-model / synchronous** tiles (**`PdfiumBackend::renderPageTile`** on the shared **`FPDF_DOCUMENT`**, GUI thread with **`m_mutex`** held) call **`FPDF_RenderPageBitmap`** using the same full-page device size **`iw` × `ih`** as a **`0, 0, iw, ih`** pass, but with **negative** **`start_x` / `start_y`** (`-tr.x()`, `-tr.y()` for the intersected tile rectangle in full-page device space) so the tile’s device rectangle lands at `(0,0)` in the tile bitmap—no full-page intermediate buffer. **Detached / async-isolated** tiles use **`pdfiumRenderPageTileIsolated`** in **`src/pdfium_tile_raster.cpp`**, which still calls **`FPDF_RenderPageBitmapWithMatrix`** with an affine built from **`FPDF_PageToDevice`** sampling, translated and clipped to the tile; that matrix path matches PDFium’s composition with the page display matrix (the plain bitmap API’s transform interaction is wrong for this case on the isolated path—see comments in `pdfium_backend.cpp`).
...
- **Mid-tile cooperative cancel** — raster entry points are not interrupted mid-call (**`FPDF_RenderPageBitmap`** on the live-model sync tile path; **`FPDF_RenderPageBitmapWithMatrix`** on PDFium’s isolated async path); cancellation is **generation-based** after the call returns.
