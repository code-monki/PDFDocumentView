...
Path resolution: explicit **`argv[1]`** when that file exists; otherwise, if **`gnu-c-language-manual.pdf`** existed at **configure** time in the **source tree root**, the build defines **`PDFDOCUMENTVIEW_EXAMPLE_DEFAULT_PDF`**; next it tries **`gnu-c-language-manual.pdf`** in the **current working directory**; otherwise no document. You may always pass a path:
...
That GNU manual is normally under the [GNU Free Documentation License 1.3](https://www.gnu.org/licenses/fdl-1.3.html); it is convenient for local testing but **too large** for typical redistribution—ship a **small synthetic** or clearly licensed tiny PDF instead. Curated binaries will live under **`tests/fixtures/`** (see `tests/fixtures/README.md`); policy today is README-only until an approved fixture exists.
